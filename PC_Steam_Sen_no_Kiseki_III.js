...
console.warn("- When viewing the details of a master quartz, only the last one will be extracted.");
...
(function () { // Also some items in the inventory
    ...
    Interceptor.attach(address, function (args) {
        ...
        if (masterQuartzAbility !== previousMasterQuartzAbility) {
            previousMasterQuartzAbility = masterQuartzAbility;
            masterQuartzAbility = masterQuartzAbility.replace(/#\d{1,3}[a-zA-Z]/g, '');
