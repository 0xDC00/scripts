...
(function () { 
    const systemMessage2Sig = 'e8??????????8b8b98000000??8b01ba1d800000ff505033d2??8b8b98000000e8??????????8b87c8000000??85c075??0f57f6eb??f30f10b004030000??8b05??????????8b4820??8d7820??85c974????8b01ba01000000ff10??892fb9e0000000??833d????????0074??e8????????eb??e8??????????8bd8??8944??????85c074??33d2??b8e0000000??8bc8e8??????????33c9ba02000000??b81b000000??8bcbe8??????????8be8eb????8bed??892f??8b05??????????8b4808??8b5908ba9f85fb41??8d0d????????e8????????8bd0??8bcbe8??????????8bd0??33c0??8bcde8??????????8b05??????????8b4808??8b5908ba9f85fb41??8d0d????????e8????????8bd0??8bcbe8??????????8bd0??b801000000??8bcde8????????f30f5835????????f3??0f2cc6??896c????baffffffff??b903000000??8bcde8??????????8b866004000083a0bc000000fe??8b8e600400008b811c020000898114020000c781180200000000803f??c78120020000cdcc4c3e??89a928020000eb??8b86e802000085c07e??ffc88986e8020000eb????8b8160040000f30f1080240200000f2f802002000072????8b470825006000003d0020000075??c781ec0400002e000000';
...
(function () { 
    const systemMessage3Sig = 'e8??????????8b8b98000000ba01800000??8b01ff5050??8b8b9800000033d2e8??????????8b876004000083a0bc000000fe??8b8f600400008b811c020000898114020000c781180200000000803f??c78120020000cdcc4c3e89b128020000e9??????????8b05??????????8b8898000000??85c974????6381a4000000??83c004??8d0440f7448140fbffffff0f85??????????8b8f60040000f30f1081240200000f2f81200200000f82????????8b811c020000??8d2d????????33f689811402000089b118020000??8d3d????????89b1280200008bde??c78120020000cdcc4c3e??8b07??c787ec04000032000000';
...
(function () { 
    const systemMessage4Sig = 'e8 ?? ?? ?? ?? ?? 8b 38 ?? 85 ff 74 ?? ?? 3b ?? ?? 74 ?? ?? 8b d5 ?? 8b cf e8 ?? ?? ?? ?? 8b d8 8b d0 ?? 8d ?? 17 e8 ?? ?? ?? ?? ?? 8b c3 ?? 8b d7 ?? 8b ?? 17 e8 ?? ?? ?? ?? ?? 8b ?? ?? c6 04 18 00 ?? 8b ?? ?? ?? 85 c9 74 ?? ?? 8d ?? 07';
...
(function () { 
    const systemMessage5Sig = 'e8 ?? ?? ?? ?? ?? 8b d8 ?? 8b 8f 98 00 00 00 ?? 8b 11 ff 52 60 ?? 8b 8f 98 00 00 00 c7 81 e0 00 00 00 00 00 80 bf ?? c7';
    var results = Memory.scanSync(__e.base, __e.size, systemMessage5Sig);
    // console.warn('\nMemory.scanSync() result: \n' + JSON.stringify(results));

    if (results.length === 0) {
        console.error('[systemMessage5Pattern] Hook not found!');
        return;
    }

    const address = results[0].address;
    console.log('[systemMessage5Pattern] Found hook', address);
    Interceptor.attach(address, function (args) {
        // console.warn("in: systemMessage5");
